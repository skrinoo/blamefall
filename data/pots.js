 ...
 * 只有「自由输入」才需要实时调用 AI。
 *
 * targetRole（可选）：标注某条快速理由「在怪谁」——self(怪自己) / npc(怪具体的人) /
 *   institution(怪制度·无主体外部力) / any(泛化谁都能圆)。引擎的契合度 fit 用它判断
 *   理由是否对上甩锅对象（命中 +8 / 错配 -12，见 engine/fallback.js）。
 *   未标注的类型走 TYPE_DEFAULT_ROLE 先验（事实型/反向型→npc，情感型→self，
 *   转移型→institution，荒诞型→any）；只有偏离先验的才在这里覆盖。
 *   AI 生成的锅（api/genpot.mjs）整体自带 targetRole，不依赖此默认。
 *
...
(function (root, factory) {
  ...
  return [
    ...
    },
    {
      ...
      targetRole: { "事实型": "institution", "反向型": "institution" }
    },
    {
      id: "ticai",
      scene: "教务选课",
    ...
    },
    {
      ...
      targetRole: { "事实型": "self", "反向型": "institution" }
    },
